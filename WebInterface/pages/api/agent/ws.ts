...
function resolveWorkspaceRoot(): string {
  ...
  if (fs.existsSync(path.join(cwd, "agents", "main.py"))) {
    return cwd;
  }

  const parent = path.resolve(cwd, "..");
  if (fs.existsSync(path.join(parent, "agents", "main.py"))) {
    return parent;
  }

  throw new Error("Could not locate agents/main.py from current runtime path.");
}
...
function handleConnection(ws: WebSocket) {
  ...
  ws.on("message", (raw) => {
    ...
    void (async () => {
      ...
      const args = ["-u", "-m", "agents.main", brief, "--rounds", String(rounds), "--interactive"];
