...
function resolveWorkspaceRoot(): string {
  ...
  const cwdHasAgents = fs.existsSync(path.join(cwd, "agents", "main.py"));
  if (cwdHasAgents) {
    return cwd;
  }

  const parent = path.resolve(cwd, "..");
  const parentHasAgents = fs.existsSync(path.join(parent, "agents", "main.py"));
  if (parentHasAgents) {
    return parent;
  }

  throw new Error("Could not locate agents/main.py from current Next.js runtime path.");
}
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const args = ["-u", "-m", "agents.main", brief, "--rounds", String(rounds)];
