...
async def generate_segment_variant(
    ...
    emit_in_pipeline(session_id, "Bandit Engine", f"Selected angle '{angle.upper()}' for tier {tier}", "Choosing psychological incentive")
    
    # 3. War Room generates and Digital Twin tests (Loop up to 3 times)
    variant = None
    for attempt in range(3):
        print(f"      [War Room] Generating draft {attempt+1}...")
        emit_in_pipeline(session_id, "War Room (Copy & Psych)", f"Drafting persuasive variant (Attempt {attempt+1})", "Collaborative drafting")
        ...
        emit_in_pipeline(session_id, "Twin Simulator", f"Tested draft on 5 synthetic personas: {clicks} Clicks, {opens} Opens", "Evaluating Bayesian kill rule")
        
        if not twin_engine.bayesian_kill_rule(twin_results):
            # Survived the kill rule!
            emit_in_pipeline(session_id, "Orchestrator", f"Draft survived the kill rule! Approving variant.", "Locking final email")
            variant = draft_variant
            break
        else:
            emit_in_pipeline(session_id, "Twin Simulator", "Kill Rule triggered due to low engagement. Regenerating draft...", "Rejecting variant")
...
async def generate_content(state: WorkflowState) -> dict:
    ...
    for seg in segments:
        ...
        emit_in_pipeline(session_id, "Orchestrator", f"Activating War Room for {seg['segment_name']} segment", "Delegating to Multi-Agent cluster")
        
        variant = await generate_segment_variant(brief, strategy, seg, session_id=session_id)
